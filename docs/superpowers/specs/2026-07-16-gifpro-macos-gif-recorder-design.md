...
4. App 显示可调整的红色选区边框和底部悬浮控制条。控制条包含：
   - 分辨率：1× 或 2×；默认 1×。例如，Retina 显示器上的 300 × 200 pt 选区分别输出 300 × 200 px 和 600 × 400 px。若显示器的 backing scale 为 1，禁用 2×，避免无意义放大。
...
7. GIFpro 完成临时 GIF 后创建预览面板，并立即在该面板上方打开系统“另存为”。预览面板使用 `QLPreviewView` 播放临时 GIF，同时显示尺寸、时长和文件大小。
8. 用户取消“另存为”时，预览面板继续保留临时 GIF，并提供“另存为”“重新录制”和“丢弃”三个操作。“重新录制”和“丢弃”都会先删除临时 GIF；前者随后进入框选，后者返回空闲状态。
9. 保存成功后，GIFpro 关闭内部预览、删除临时副本，并用系统 Quick Look 打开成品。
...
- `SaveAndPreviewController`：用 `QLPreviewView` 预览临时 GIF，显示 `NSSavePanel`，处理重新保存、重新录制和丢弃，并在保存后调用系统 Quick Look。
...
recording → finalizing → previewReady → awaitingSave
awaitingSave → savedPreview → idle
awaitingSave → previewReady（取消另存为）
previewReady → awaitingSave（再次另存为）
previewReady → selecting（重新录制）或 discarding → idle
...
- `width` / `height`：选区逻辑尺寸乘以所选倍率；倍率不超过显示器实际 backing scale。Retina 显示器上的 300 × 200 pt 选区在 1× 时输出 300 × 200 px，在 2× 时输出 600 × 400 px。
...
| 用户取消另存为 | 保留临时 GIF 和预览面板；允许再次另存为、重新录制或丢弃 |
...
- 另存为成功、取消后再次保存、取消后重新录制、丢弃、同名覆盖失败和无写入权限目录。
